...
app.post("/UsersReg", (req, res) => {
  ...
  const SQL =
    "INSERT INTO clientes (ID,primerNombre, segundoNombre, primerApellido, segundoApellido, lugarNacimiento, fechaNacimiento, correoElectronico, direccionCorrespondencia, contraseña, usuario, IdCargo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
...
app.post("/usersAdminReg", (req, res) => {
  ...
  const SQL =
    "INSERT INTO clientes (ID,primerNombre, segundoNombre, primerApellido, segundoApellido, fechaNacimiento, correoElectronico, lugarNacimiento, contraseña, usuario, IdCargo) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
...
app.post("/BookAdd", (req, res) => {
  const setIdLibro = req.body.IdLibro;
  const setTituloLibro = req.body.TituloLibro;
  const setAuthorLibro = req.body.AuthorLibro;
  const setNumeroPaginas = req.body.NumeroPaginas;
  const setEditorial = req.body.Editorial;
  const setCantidadLibro = req.body.CantidadLibro;
  const setAñoPublicacion = req.body.AñoPublicacion;
  const setGeneroLibro = req.body.GeneroLibro;
  const setEstadoLibro = req.body.EstadoLibro;

  const SQL =
    "INSERT INTO Libro (idLibro,tituloLibro,	autorLibro,	numeroPaginas,	editorial,	cantidadLibro,	añoPublicación,	generoLibro,	estadoLibro) VALUES (?,?,?,?,?,?,?,?,?)";
